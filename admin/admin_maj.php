				...
				<head>
					...
					if (isset($_POST['maj']) || isset($_GET['force_maj']))
					{
						...
						if ($version_old <= "3.5")
						{
							...
							if ($req == -1){
								$result_inter .= traite_requete("INSERT INTO ".TABLE_PREFIX."_setting VALUES ('logo', 'logo.jpg');");
							}else{
								$result_inter .= traite_requete("UPDATE ".TABLE_PREFIX."_setting SET VALUE = 'logo.jpg' WHERE NAME = 'logo';");
							}

							//Modification du champ 'company'
							$req = grr_sql_query1("SELECT VALUE FROM ".TABLE_PREFIX."_setting WHERE NAME='company';");
							if ($req == -1){
								$result_inter .= traite_requete("INSERT INTO ".TABLE_PREFIX."_setting VALUES ('company', 'Asac Tregor');");
							}else{
								$result_inter .= traite_requete("UPDATE ".TABLE_PREFIX."_setting SET VALUE = 'Asac Tregor' WHERE NAME = 'company';");
							}
					...
					if ($version_old <= "3.6"){
							...
							$result_inter .= traite_requete("Create table if not exists ".TABLE_PREFIX."_files(id int not null auto_increment, id_entry int, file_name varchar(50), public_name varchar(50), Primary key (id));");
